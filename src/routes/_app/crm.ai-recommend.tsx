...
export const Route = createFileRoute("/_app/crm/ai-recommend")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Calculate UPS backup time across load scenarios for predefined battery sizes.",
      },
    ],
  }),
});
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Estimate runtime across 25/50/75/100% load for common battery sizes."
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-3 [&>*]:space-y-2">
            <div>
            ...
            <div>
              ...
              <Input
                ...
                step="1"
                value={dcBus}
                onChange={(e) => setDcBus(e.target.value)}
              />
            </div>
            <div>
            ...
            <div className="col-span-2 md:col-span-4 border-t pt-3 flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                <Switch
                  ...
                  onCheckedChange={setIncludeCharger}
                />
                <Label
                  htmlFor="charger-toggle"
              ...
              {includeCharger && (
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    step="0.1"
                    value={chargerAmp}
                    onChange={(e) => setChargerAmp(e.target.value)}
                    className="w-28 h-8"
                  />
                </div>
              )}
            ...
            <div className="col-span-2 md:col-span-4 grid grid-cols-2 md:grid-cols-4 gap-2 pt-2 text-sm">
              ...
              <Stat
                label="Load 25% (W)"
                value={load100 > 0 ? loadFor(25).toFixed(0) : "—"}
                highlight
              />
            </div>
          </CardContent>
        ...
        <Card className="border-primary/40 bg-primary/5">
          ...
          <CardContent className="space-y-2 text-sm">
            <div className="text-xs text-muted-foreground">Load (W) = KVA × 1000 × PF</div>
      ...
      {errors.length > 0 && (
        <div className="print:hidden border border-destructive/40 bg-destructive/5 text-destructive rounded-md px-3 py-2 text-sm flex items-center gap-2">
                  ...
                  <tr>
                    <td colSpan={6} className="py-6">
                      <EmptyState
                        icon={Calculator}
                        title="No results"
                        description="Enter KVA, DC Bus Voltage, and Efficiency to see backup times."
                      />
                    </td>
                  </tr>
                ) : (
                  rows.map((r) => (
                    ...
                    >
                      ...
                      {LOAD_LEVELS.map((lvl) => (
                        <td
                          ...
                          className={cn(
                            ...
                            lvl === 100 && "font-semibold",
                          )}
                        >
                          {r.backup[lvl] === -1 ? (
                            <span className="text-[11px] text-emerald-600">
                              charger supports load
                            </span>
                          ) : (
                            `${r.backup[lvl]} min`
                          )}
                      ...
                      <td className="px-3 py-2 text-center">
                        {r.recommended && (
                          <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium bg-primary/10 text-primary ring-1 ring-primary/20">
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div
      className={`rounded border px-3 py-2 ${highlight ? "bg-primary/10 border-primary/40" : "bg-background"}`}
    >
