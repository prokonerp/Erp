...
function SalesOrderDetail() {
  ...
  const setStatus = async (s: SoStatus) => {
    // Guard: invoiced status is only set by createInvoiceFromSalesOrder
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          <Button size="sm" variant="outline" onClick={() => runDc()} disabled={dcBusy}>
            <Truck className="h-4 w-4 mr-1" />
            Create Delivery Challan
          </Button>
          <Button size="sm" onClick={() => runInvoice()} disabled={invoiceBusy}>
          ...
          <table className="w-full text-sm">
            ...
            <tfoot className="border-t bg-muted/30">
              <tr>
                ...
                <td className="p-2 text-right">{inr(so.subtotal)}</td>
              </tr>
              {so.discount > 0 && (
                <tr>
                  <td colSpan={7} className="p-2 text-right text-muted-foreground">
                  ...
                  <td className="p-2 text-right text-red-600">−{inr(so.discount)}</td>
                </tr>
              )}
              {so.cgst > 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr(so.cgst)}</td>
                </tr>
              )}
              {so.sgst > 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr(so.sgst)}</td>
                </tr>
              )}
              {so.igst > 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr(so.igst)}</td>
                </tr>
              )}
              {so.cess > 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr(so.cess)}</td>
                </tr>
              )}
              {(so as any).shipping_charges > 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr((so as any).shipping_charges)}</td>
                </tr>
              )}
              {(so as any).adjustment !== 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr((so as any).adjustment)}</td>
                </tr>
              )}
              {(so as any).tcs_amount > 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr((so as any).tcs_amount)}</td>
                </tr>
              )}
              {so.round_off !== 0 && (
                <tr>
                  ...
                  <td className="p-2 text-right">{inr(so.round_off)}</td>
                </tr>
              )}
              <tr className="font-semibold">
                ...
                <td className="p-2 text-right">{inr(so.total)}</td>
              </tr>
            </tfoot>
          </table>
          {so.total_in_words && (
