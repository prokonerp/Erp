...
export const Route = createFileRoute("/_app/ims/defective-tags")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Generate, view and print defective tags from Defective Stock IN records.",
      },
      { property: "og:title", content: "Defective Tags — Prokon Inventory" },
      {
        property: "og:description",
        content: "Generate and print defective tags for defective stock received.",
      },
...
function DefectiveTagsPage() {
    ...
    setRecords((prev) =>
      prev.map((r) => {
        ...
        const t = created.find(
          (c) =>
            normKey(c.model_no) === normKey(r.model_no) &&
            normKey(c.serial_no) === normKey(r.serial_no),
        );
  ...
  const activeTab =
    tab && (tab === "register" || aspTabs.includes(tab)) ? tab : aspTabs[0] || "register";
    ...
    <div className="space-y-4">
      ...
      >
        <DialogContent className="max-w-[900px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Defective Tag Preview — {preview?.length || 0} tag(s), A4 landscape, 4 per page
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={!!askDc} onOpenChange={(o) => !o && setAskDc(null)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAskDc(null)}>
              No
            </Button>
            <Button
              onClick={() => {
...
function AspTab({
  ...
  const filtered = useMemo(() => {
    const s = deferredQ.toLowerCase().trim();
  ...
  }, [rows, deferredQ, showGenerated]);

  useEffect(() => {
    setPage(0);
  }, [deferredQ, showGenerated]);
    ...
    try {
      ...
      const existing = allTags.filter((t) =>
        chosen.some(
          (r) =>
            r.tag_generated &&
            norm(t.model_no) === norm(r.model_no) &&
            norm(t.serial_no) === norm(r.serial_no),
        ),
      );
      setSel({});
      ...
      toast.error(
        e?.message?.includes("duplicate")
          ? "A tag already exists for one of the selected records"
                ...
                <tr className="text-left">
                  <th className="p-2 w-8">
                    <Checkbox
                      ...
                      onCheckedChange={(v) =>
                        setSel(v ? Object.fromEntries(filtered.map((r) => [r.key, true])) : {})
                      }
                    />
                  </th>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td className="p-4 text-muted-foreground" colSpan={12}>
                      No pending defective items for this ASP.
                    </td>
                  </tr>
                ) : (
                  paged.map((r) => (
                    ...
                    >
                      ...
                      <td className="p-2 whitespace-nowrap">
                        {(() => {
                          ...
                          if (d === null) return "—";
                          return (
                            <span className={d > 10 ? "text-destructive font-bold" : ""}>{d}</span>
                          );
                        })()}
                      </td>
                          ...
                          <div className="flex items-center gap-1 whitespace-nowrap">
                            ...
                            {(() => {
                              const t = allTags.find(
                                (x) =>
                                  normKey(x.model_no) === normKey(r.model_no) &&
                                  normKey(x.serial_no) === normKey(r.serial_no),
                              );
                              return t ? (
                                <Button
                                  size="sm"
        ...
        <CardContent className="py-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Button disabled={!selectedRows.length || saving} onClick={generate}>
              <Printer className="h-4 w-4 mr-1" />
              Print Tag{selectedRows.length === 1 ? "" : "s"}
...
function RegisterTab({
  ...
  const filtered = useMemo(() => {
    ...
    const rows = tags.filter((t) => {
      if (deferredStatus === "printed" && !t.printed_at) return false;
      if (deferredStatus === "not_printed" && t.printed_at) return false;
      if (deferredAspFilter !== "all" && (t.asp_code || "") !== deferredAspFilter) return false;
      if (!s) return true;
      return [
        t.tag_no,
        t.oem_case_id,
  ...
  }, [tags, deferredQ, deferredStatus, deferredAspFilter, sortKey, sortAsc]);

  useEffect(() => {
    setPage(0);
  }, [deferredQ, deferredStatus, deferredAspFilter]);
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <Input
            ...
            placeholder="Tag no / OEM case / Oracle / Model / Serial / Customer / ASP / Engineer…"
            value={q}
            onChange={(e) => setQ(e.target.value)}
          />
          ...
          <Select value={aspFilter} onValueChange={setAspFilter}>
            <SelectTrigger>
              <SelectValue placeholder="All ASPs" />
            </SelectTrigger>
            <SelectContent>
        ...
        <CardContent className="p-0">
          ...
          >
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10 bg-muted shadow-sm">
                <tr className="text-left">
                  <th className="p-2 cursor-pointer" onClick={() => toggleSort("tag_no")}>
                    Tag No
                  </th>
                  <th className="p-2 cursor-pointer" onClick={() => toggleSort("tag_date")}>
                    Generated
                  </th>
                  <th className="p-2">OEM Case ID</th>
                  <th className="p-2">Oracle Order</th>
                  <th className="p-2 cursor-pointer" onClick={() => toggleSort("model_no")}>
                    Model · Serial
                  </th>
                  <th className="p-2 cursor-pointer" onClick={() => toggleSort("customer_name")}>
                    Customer
                  </th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td className="p-4 text-muted-foreground" colSpan={12}>
                      No tags dispatched to OEM yet — tags appear here once a DC to OEM is generated
                      for them.
                    </td>
                  </tr>
                ) : (
