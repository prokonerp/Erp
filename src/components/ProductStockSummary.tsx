...
export function ProductStockSummary({
    ...
    for (const r of stock) {
      ...
      if (!g) {
        g = {
          key,
          oem: r.oem,
          model: product.model || r.part_model_no || "—",
          total: 0,
          available: 0,
          reservedIssued: 0,
          defective: 0,
  ...
  const filtered = useMemo(() => {
    ...
    const list = term
      ? groups.filter((g) =>
          [g.model, g.oem].filter(Boolean).some((v) => String(v).toLowerCase().includes(term)),
        )
      : groups;
  ...
  function th(label: string, key: SortKey, align = "left") {
    ...
    return (
      <th
        className={`p-2 cursor-pointer select-none ${align === "right" ? "text-right" : "text-left"}`}
        onClick={() =>
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <Input
          placeholder="Search by product name, model no or OEM…"
          value={q}
          ...
          <table className="w-full text-sm table-fixed">
            <thead className="sticky top-0 z-10 bg-muted shadow-sm">
              <tr>
                ...
                <th className="p-2 text-left bg-muted">Warehouses</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                  ...
                  return (
                    <Fragment key={g.key}>
                      ...
                      >
                        ...
                        <td className={`p-2 text-right ${qtyCellClass(g.available)}`}>
                          {g.available}
                        </td>
        ...
        <p className="text-xs text-muted-foreground">
          Low stock threshold: Available &lt; {LOW_STOCK_THRESHOLD}. {warehouses.length}{" "}
          warehouse(s) tracked. Showing {displayGroups.length} of {filtered.length} groups.
        </p>
      </CardContent>
    </Card>
  );
