...
const WAITING_STATUSES = new Set(["Waiting for Parts", "Parts Received", "Under Observation"]);
const OPEN_STATUSES = new Set(["New", "Call Log"]);

const ENGINEER_CAP = 2000; // cap 2k to protect main thread — bounded fetch with count:exact
...
export function EngineerWorkloadSection() {
  ...
  const [sortKey, setSortKey] = useState<"active" | "today" | "carry" | "oldest" | "high">(
    "active",
  );
  ...
  const deferredPriorityF = useDeferredValue(priorityF);
  const deferredStatusF = useDeferredValue(statusF);
  const isStale =
    deferredEngineerQ !== engineerQ ||
    deferredPriorityF !== priorityF ||
    deferredStatusF !== statusF;

  const load = async () => {
    // Bounded fetch: page 500 with hard cap 2k + count:exact. Prevents 21k sequential awaits.
    const all: T[] = [];
    const size = 500;
    let count: number | null = null;
    for (let from = 0; from < ENGINEER_CAP; from += size) {
      const {
        data,
        error,
        count: c,
        ...
        .from("tickets")
        .select(
          "id,case_id,status,priority,assigned_engineer_name,assigned_at,created_at,closed_at,customer_name,location",
          { count: "exact" },
        )
        .eq("is_deleted", false)
        .order("created_at", { ascending: false })
        .range(from, from + size - 1);
      if (count === null && typeof c === "number") count = c;
      if (error) break;
      const batch = (data || []) as T[];
      all.push(...batch);
      if (batch.length < size) break;
      // cap 5k to protect main thread (spec: 2k cap, loop capped at 5k safety)
      if (from + size >= ENGINEER_CAP) break; // cap 2k to protect main thread
    }
    setTotalCount(count);
    setRows(all);
  };
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      if (deferredPriorityF !== "all" && (r.priority || "") !== deferredPriorityF) return false;
      if (deferredStatusF !== "all" && r.status !== deferredStatusF) return false;
      return true;
    });
  }, [rows, deferredPriorityF, deferredStatusF]);
  ...
  const summary = useMemo(() => {
    ...
    const closedToday = r.filter(
      (t) => t.status === "Closed" && isToday(t.closed_at, startToday, endToday),
    );
    const engineersWithActive = new Set(
      active.map((t) => t.assigned_engineer_name || "Unassigned"),
    );
    const avg = engineersWithActive.size
      ? Math.round((active.length / engineersWithActive.size) * 10) / 10
      : 0;
    return {
      today: today.length,
      carry: carry.length,
  ...
  const overdue = useMemo(() => {
    return filteredRows
      .filter(
        (t) => t.status !== "Closed" && t.status !== "Cancelled" && ticketElapsedHours(t) > 24,
      )
  ...
  const priorityDist = useMemo(() => {
    ...
    for (const t of active)
      if (t.priority && buckets[t.priority] !== undefined) buckets[t.priority]++;
    return [
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <SumCard
          label="Carry-over"
          value={summary.carry}
          tone={summary.carry ? "alert" : "muted"}
          search={{ scope: "carry" }}
        />
        <SumCard
          label="Total Active"
        ...
        <CardHeader className="py-3 px-4 border-b bg-muted/30">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              ...
              {totalCount !== null && totalCount > ENGINEER_CAP && (
                <span className="text-[10px] font-normal text-muted-foreground">
                  capped {ENGINEER_CAP} of {totalCount.toLocaleString()}
                </span>
              )}
            </CardTitle>
            <div className="flex items-center gap-2 flex-wrap">
              <Input
                ...
                value={engineerQ}
                onChange={(e) => setEngineerQ(e.target.value)}
                className="h-8 w-40 text-xs"
              />
              {isStale && <span className="text-[10px] text-muted-foreground">Updating…</span>}
          ...
          {rows === null ? (
            ...
            >
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10 bg-muted text-[10px] uppercase tracking-wide text-muted-foreground shadow-sm">
                  <tr>
                    <th className="text-left p-2 w-6 bg-muted"></th>
                    <th className="text-left p-2 bg-muted">Engineer</th>
                    <th className="text-right p-2 bg-muted">Today</th>
                    <th className="text-right p-2 bg-muted">Carry-over</th>
                    <th className="text-right p-2 bg-muted">Active</th>
                    <th className="text-right p-2 bg-muted">In Prog.</th>
                    <th className="text-right p-2 bg-muted">Open</th>
                    ...
                    return (
                      <React.Fragment key={e.name}>
                        ...
                        >
                          ...
                          <NumCell
                            value={e.active}
                            search={{ scope: "active", engineer: e.name }}
                            strong
                          />
                          <NumCell
                            value={e.inProgress}
                            search={{ status: "In Progress", engineer: e.name }}
                          />
                          <NumCell value={e.open} search={{ scope: "active", engineer: e.name }} />
                          <NumCell
                            value={e.waiting}
                            search={{ status: "Waiting for Parts", engineer: e.name }}
                          />
                          <NumCell
                            value={e.closedToday}
                            search={{ scope: "closedToday", engineer: e.name }}
                          />
                          <NumCell
                            value={e.high}
...
function SumCard({
  ...
  label: string;
  value: number | string;
  tone: "active" | "alert" | "positive" | "neutral" | "muted";
  ...
  const body = (
    <div
      className={`rounded-md border border-l-4 bg-card px-3 py-3 ${bar.split(" ")[0]} hover:shadow-sm transition`}
    >
  ...
  return search ? (
    <Link to="/tickets" search={search as any}>
      {body}
    </Link>
  ) : (
    body
...
function CapacityBadge({ count }: { count: number }) {
  if (count <= CAPACITY_GREEN)
    return <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100">🟢 Light</Badge>;
  if (count <= CAPACITY_YELLOW)
  ...
  return (
    <div>
      <div className="text-xs text-muted-foreground mb-2 flex items-center justify-between">
        ...
        <Link
          to="/tickets"
          search={{ engineer, scope: "active" } as any}
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {tickets.map((t) => {
              ...
              const sla =
                hrs > 48
                  ? { text: "Breached", cls: "bg-red-100 text-red-800" }
                  : hrs > 24
                    ? { text: "At Risk", cls: "bg-amber-100 text-amber-800" }
                    : { text: "On Track", cls: "bg-emerald-100 text-emerald-800" };
              return (
                <tr key={t.id} className="border-t hover:bg-muted/30">
                  <td className="p-2">
                    <Link
                  ...
                  <td className="p-2">
                    <Badge className={sla.cls}>{sla.text}</Badge>
                  </td>
                </tr>
              );
            })}
