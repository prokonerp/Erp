...
export function VendorPicker({
  ...
  onChange,
  required,
  placeholder = "Search vendors / OEMs…",
  label = "Vendor",
  className,
}: Props) {
  ...
  const rows = (allVendors ?? []) as Vendor[];
          ...
          <CommandList>
            <CommandEmpty>
              <div className="py-4 px-3 text-sm space-y-2">
                ...
                >
                  <Plus className="h-3.5 w-3.5" /> Add in Masters{" "}
            ...
            <CommandGroup heading={`${rows.length} ${label.toLowerCase()}s`}>
              {rows.map((v) => {
                const blob = [v.name, v.contact_name, v.phone, v.gstin]
                  .filter(Boolean)
                  .join(" ")
                  .toLowerCase();
                return (
