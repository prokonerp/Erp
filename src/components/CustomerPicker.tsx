  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="p-0 w-[--radix-popover-trigger-width] min-w-[320px]" align="start">
        <Command
          filter={(val, search) => {
            // val is the customer id; we stash searchable text via keywords on each item
            return val.toLowerCase().includes(search.toLowerCase()) ? 1 : 0;
          }}
        >
          <CommandInput placeholder="Search by name, mobile, GST…" />
          <CommandList>
            <CommandEmpty>
          ...
          <div className="border-t p-2">
            ...
            >
              <Plus className="h-3.5 w-3.5" /> Add New Customer in Masters
