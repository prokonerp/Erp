...
export function SerialsManager({ product }: { product: Product }) {
  ...
  const load = async () => {
    const [{ data: ser }, { data: ven }, { data: cust }] = await Promise.all([
      ...
      <div className="border rounded-md overflow-x-auto">
        <Table>
          ...
          <TableBody>
            ...
            {filtered.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">No serials. Click <b>Add Serial</b>.</TableCell></TableRow>}
          </TableBody>
        </Table>
      </div>
